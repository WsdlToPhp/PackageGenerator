...
class ApiTaxType extends AbstractStructBase
{
    ...
    /**
     * This method is responsible for validating the values passed to the method setTaxDescription
    ...
    public function setType($type = null)
    {
        ...
        if (!\Api\EnumType\ApiAmountDeterminationType::valueIsValid($type)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Api\EnumType\ApiAmountDeterminationType', is_array($type) ? implode(', ', $type) : $type, implode(', ', \Api\EnumType\ApiAmountDeterminationType::getValidValues())), __LINE__);
        }
        $this->Type = $type;
        return $this;
    }
