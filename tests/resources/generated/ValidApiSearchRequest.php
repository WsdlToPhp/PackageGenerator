...
class ApiSearchRequest extends AbstractStructBase
{
    ...
    public function setAdult($adult = null)
    {
        ...
        if (!\Api\EnumType\ApiAdultOption::valueIsValid($adult)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Api\EnumType\ApiAdultOption', is_array($adult) ? implode(', ', $adult) : $adult, implode(', ', \Api\EnumType\ApiAdultOption::getValidValues())), __LINE__);
        }
        $this->Adult = $adult;
        return $this;
    }
